...
public class CompanyRegistrationCommand<T>(string firstName,
									 string secondName,
									 string thirdName,
									 string phone,
									 string email,
									 string password,
									 EntityType type,
									 byte[] logo) : IRequest<T> where T : CompanyDto
{
...
public class CompanyRegistrationCommandHandler<T>(IEntityRepository entitiesRepository, IPasswordHash passwordHash, IMapper mapper) : IRequestHandler<CompanyRegistrationCommand<T>, T> where T : CompanyDto
{
	...
	public async Task<T> Handle(CompanyRegistrationCommand<T> request, CancellationToken cancellationToken)
	{
